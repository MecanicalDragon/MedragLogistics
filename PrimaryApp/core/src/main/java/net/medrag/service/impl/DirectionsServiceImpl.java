...
//            10560 limit of minutes
            ...
            try {
                Float depX = Float.valueOf(departure.getCoordinatesX());
                Float depY = Float.valueOf(departure.getCoordinatesY());
                Float destX = Float.valueOf(destination.getCoordinatesX());
                Float destY = Float.valueOf(destination.getCoordinatesY());

                double difX = Math.abs(depX - destX);
                double difY = Math.abs(depY - destY);

                int distance = (int) (Math.sqrt(Math.pow(difX, 2) + Math.pow(difY, 2)) * 111);
                int minutes = distance / 80 * 60;
                return new Integer[]{distance, minutes};
...
//        Counting the percent of delivery
        ...
        return 100 - (100 * leftTime[0] / (totalTime[0] == 0 ? 1 : totalTime[0]));
    }
